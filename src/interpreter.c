...
RTExpr *eval_RTExpr(RTExpr *rt_expr, Scope *scope) {
	RTExpr *return_rt_expr, *rt_expr_left, *rt_expr_right;
	RTValue *rt_value;
	RTAlloc *alloc = scope->alloc;
	...
	switch (rt_expr->type) {
		...
		case RT_VAR:
			...
			return eval_RTExpr(return_rt_expr, scope);

		case RT_CALL: return NULL;
		case RT_UNARY:
			return_rt_expr=eval_RTExpr(rt_expr->value, scope);
		...
		case RT_BINOP:
			rt_expr_left=eval_RTExpr(rt_expr->left, scope);
			if (rt_expr_left->type!=RT_VALUE)
				return rt_expr;

			rt_expr_right=eval_RTExpr(rt_expr->right, scope);
...
bool eval_expr(Interpreter *inter) {
	...
	Scope *scope = inter->glob_scope;
	if (_compare_var(node, L"alloc")) {
		print_RTAlloc(scope->alloc);
		return true;
	...
	print_ASTNode(node);
	RTExpr *rt_expr=eval_ASTNode(scope->alloc, node);
	if (rt_expr==NULL)
		return true;

	rt_expr=eval_RTExpr(rt_expr, scope);
	putwchar(L'\n');
	print_RTExpr(rt_expr);
	garbage_collect(scope, 0); //test
	return true;
}
