...
Scope *new_Scope(Scope *parent, wchar_t *name) {
	...
	scope->vars=new_HashMap(64, &stdcomp_wstring, &stdhash_string);
	scope->alloc=new_RTAlloc(64);
	return scope;
}
...
void free_Scope(Scope *scope) {
	...
	free_HashMap(scope->vars);
	free_RTAlloc(scope->alloc);
	free(scope->name);
	free(scope);
}
...
RTExpr *Scope_get_var(Scope *scope, const wchar_t *varname) {
	...
	RTExpr *rt_expr = HashMap_get(scope->vars, varname);
	if (rt_expr!=NULL)
		return rt_expr;
...
static void _mark_reachable(RTExpr *rt_expr) { //TODO: handle cycles
	rt_expr->_reachable=true;
	switch (rt_expr->type) {
		case RT_CALL: return;
		case RT_UNARY:
			_mark_reachable(rt_expr->value);
			return;
		case RT_BINOP:
			_mark_reachable(rt_expr->left);
			_mark_reachable(rt_expr->right);
			return;
		case RT_SUM:
		case RT_PROD:
		case RT_INT:
...
size_t garbage_collect(Scope *scope, ushort level) {
	RTExpr *rt_expr, **objects = scope->alloc->objects;
	int i, length=scope->alloc->length, obj_count=0;
	for (i=0; i<length; i++)
		objects[i]->_reachable=false;

	hash_entry *entry;
	iterHashMap iter_hm;
	init_iterHashMap(&iter_hm, scope->vars);
	for (entry=iterHashMap_next(&iter_hm); entry!=NULL;
		 entry=iterHashMap_next(&iter_hm)) {
		_mark_reachable(entry->value);
	}
	
	for (i=0; i<length; i++) {
		rt_expr=objects[i];
		while (!rt_expr->_reachable && i<length) {
			free_RTExpr(rt_expr);
			rt_expr=objects[--length];
			obj_count++;
		}
		objects[i]=rt_expr;
	}
	
	scope->alloc->length = length;
	return sizeof(RTExpr)*obj_count;
}
